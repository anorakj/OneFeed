...
GET_MESSAGE = """SELECT id, message_info FROM (
                 ...
                 store.id, message_info FROM sync JOIN store ON sync.message_id = store.id 
                 WHERE json_extract(store.message_info, '$.source') = ? ORDER BY sync.update_time desc) t where rn = 1"""
